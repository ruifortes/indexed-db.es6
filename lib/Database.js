...
var _ReadOnlyTransaction = require("./transaction/ReadOnlyTransaction");

var _ReadOnlyTransaction2 = _interopRequireDefault(_ReadOnlyTransaction);
...
var Database = function () {
  ...
  function Database(database) {
    ...
    database.onversionchange = function (event) {
      ...
      _this[FIELDS.versionChangeListeners].forEach(function (listener) {
        try {
          listener(newVersion);
        } catch (error) {
    ...
    this.addVersionChangeListener(function () {
      if (_this[FIELDS.versionChangeListeners].size !== 1) {
        return;
      }
  ...
  _createClass(Database, [{
    key: "addVersionChangeListener",
    value: function addVersionChangeListener(listener) {
    ...
    value: function startTransaction() {
      ...
      for (var _len = arguments.length, objectStoreNames = Array(_len), _key = 0; _key < _len; _key++) {
        objectStoreNames[_key] = arguments[_key];
      }
      ...
      var transaction = new _Transaction2.default(nativeTransaction, function (objectStoreName) {
        return _this2.startReadOnlyTransaction(objectStoreName);
      });

      this[FIELDS.activeTransactions].add(transaction);
      transaction.completionPromise.catch(function () {}).then(function () {
        _this2[FIELDS.activeTransactions].delete(transaction);
      });
    ...
    key: "startReadOnlyTransaction",
    value: function startReadOnlyTransaction() {
      var _this3 = this;

      for (var _len2 = arguments.length, objectStoreNames = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
        objectStoreNames[_key2] = arguments[_key2];
      }
      ...
      var transaction = new _ReadOnlyTransaction2.default(nativeTransaction, function (objectStoreName) {
        return _this3.startReadOnlyTransaction(objectStoreName);
      });

      this[FIELDS.activeTransactions].add(transaction);
      transaction.completionPromise.catch(function () {}).then(function () {
        _this3[FIELDS.activeTransactions].delete(transaction);
      });
    ...
    key: "runTransaction",
    value: function runTransaction(objectStoreNames, transactionOperations) {
      if (typeof objectStoreNames === "string") {
        objectStoreNames = [objectStoreNames];
      }

      var transaction = this.startTransaction.apply(this, _toConsumableArray(objectStoreNames));
      return _runTransaction(transaction, objectStoreNames, transactionOperations);
    }
    ...
    key: "runReadOnlyTransaction",
    value: function runReadOnlyTransaction(objectStoreNames, transactionOperations) {
      if (typeof objectStoreNames === "string") {
        objectStoreNames = [objectStoreNames];
      }

      var transaction = this.startReadOnlyTransaction.apply(this, _toConsumableArray(objectStoreNames));
      return _runTransaction(transaction, objectStoreNames, transactionOperations);
    }
...
function _runTransaction(transaction, objectStoreNames, transactionOperations) {
  ...
  var resultPromise = transactionOperations.apply(undefined, _toConsumableArray(callbackArguments));
